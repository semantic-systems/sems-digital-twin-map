 ...
 * hidden reports (showHidden only affects the sidebar list, never the map),
 * "only new", and containment suppression. The drawn-area filter is applied
 * server-side (via PostGIS), so `dots` already excludes out-of-area points.
 * Single source of truth so anything that draws per-location map overlays (dots,
 * polygons) reads the SAME visibility decision — a location whose dot doesn't
 * survive here can't independently keep its polygon on screen either.
 */

...
/**
 * The little "(N)" counts next to each filter checkbox/chip, adjusted to
 * include the onboarding tour's example report. Those counts are computed
 * server-side (get_reports' cross-filtered facet loop) and the example is
 * excluded from every real query (build_report_query), so without this the
 * panel would silently undercount by one wherever the example currently
 * applies — e.g. showing "High (3)" while a 4th (the example) is plainly

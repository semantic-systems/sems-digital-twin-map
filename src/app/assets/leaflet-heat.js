/*
 Leaflet.heat v0.2.0, a tiny and fast heatmap plugin for Leaflet.
...
L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    _redraw: function() {
        ...
        var data = [], r = this._heat._r, size = this._map.getSize(),
            ...
            cellSize = r / 2, grid = [], panePos = this._map._getMapPanePos(),
            offsetX = panePos.x % cellSize, offsetY = panePos.y % cellSize, i, len, p, cell, x, y, j;
        for (i = 0, len = this._latlngs.length; i < len; i++) {
            ...
            if (bounds.contains(p)) {
                ...
                grid[y] = grid[y] || [];
                cell = grid[y][x];
                if (!cell) grid[y][x] = [p.x, p.y, j];
                else { cell[0] = (cell[0] * cell[2] + p.x * j) / (cell[2] + j); cell[1] = (cell[1] * cell[2] + p.y * j) / (cell[2] + j); cell[2] += j; }
            }
        }
        for (i = 0, len = grid.length; i < len; i++) {
            if (grid[i]) { for (j = 0, len = grid[i].length; j < len; j++) { cell = grid[i][j]; if (cell) { data.push([Math.round(cell[0]), Math.round(cell[1]), Math.min(cell[2], max)]); } } }
        }
